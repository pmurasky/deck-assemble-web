...
import { X, Download, FileCode, FileSpreadsheet, FileText, Check, Printer } from 'lucide-react';
import {
...
const FORMAT_OPTIONS: FormatOption[] = [
  ...
  {
    format: 'proxy-sheet',
    label: 'Proxy Sheet (Print/PDF)',
    extension: 'pdf',
    description: 'Print-optimized layout of unowned missing cards for playtesting',
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-lg w-full p-6 text-zinc-100 shadow-2xl relative">
          ...
          <div className="flex justify-end gap-3 border-t border-zinc-800 pt-4">
            ...
            >
              {selectedFormat === 'proxy-sheet' ? <Printer className="w-4 h-4" /> : <Download className="w-4 h-4" />}
              <span>{selectedFormat === 'proxy-sheet' ? 'Print / Preview Proxy Sheet' : 'Download Export'}</span>
            </button>
          </div>
        </div>
      </div>
