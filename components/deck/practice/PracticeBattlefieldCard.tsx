...
interface PracticeBattlefieldCardProps {
  card: PracticeCard;
  onToggleTap: (id: number | string) => void;
}

export function PracticeBattlefieldCard({ card, onToggleTap }: PracticeBattlefieldCardProps) {
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      aria-label={`${card.name} (${card.tapped ? 'Tapped' : 'Untapped'})`}
      onClick={() => onToggleTap(card.id)}
      onKeyDown={handleKeyDown}
      className={`p-2.5 rounded-lg border text-left transition-all max-w-[170px] flex flex-col justify-between cursor-pointer ${
        card.tapped
