...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = new URL('/api/v1/decks/import/errors', API_BASE_URL);
    ...
    if (!backendRes.ok) {
      ...
      const msg = errJson?.error?.message || errJson?.message || 'Download import errors failed';
    ...
    headers.set('content-disposition', backendRes.headers.get('content-disposition') || 'attachment; filename="import_errors.csv"');

    return new NextResponse(backendRes.body, { status: 200, headers });
  } catch (error: unknown) {
    return handleRouteError(error, 'Import errors download failed');
  }
}
