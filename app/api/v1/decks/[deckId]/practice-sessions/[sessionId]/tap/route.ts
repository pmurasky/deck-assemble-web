...
import { tapPracticeCard } from '@/lib/api/simulations';

function errorMessage(error: unknown) {
  return error instanceof Error ? error.message : 'Failed to tap card';
}
...
export async function POST(
  ...
  try {
    ...
    const data = await tapPracticeCard(id, sessionId, printingId);
